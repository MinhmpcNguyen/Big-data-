...
Silver
docker exec -it data-platform-control-plane crictl rmi spark-weather-silver:3.3.3-r1
...
kubectl create job \
  --from=cronjob/weather-s3-to-silver-cron \
  weather-s3-to-silver-manual-$(date +%s) \
  -n default
kubectl get sparkapplications -n default
kubectl get pods -n default | grep silver

kubectl delete cronjob weather-s3-to-silver-cron -n default
kubectl get cronjob weather-s3-to-silver-cron -n default
kubectl describe cronjob weather-s3-to-silver-cron -n default
kubectl get jobs -n default | grep weather-s3-to-silver

kubectl get pods -n default -w | grep weather-s3-to-silver
kubectl logs weather-s3-to-silver-nsmk4-driver -n default
kubectl logs -f weather-s3-to-silver-2j72j-driver -n default

kubectl delete sparkapplication <APP_NAME> -n default
...
kubectl exec silver-debug -n default -- ls -lt /data/silver/weather | head
kubectl get sparkapplications -n default | grep weather-s3-to-silver

kubectl exec silver-debug -n default -- ls -lt /data/silver/weather/city=Hanoi


tìm dữ liệu mới
kubectl run spark-silver-inspect \
  --rm -it \
  --restart=Never \
...
aws s3 ls s3://hust-bucket-storage/weather_silver/ --recursive | tail
...
kubectl create job \
  --from=cronjob/silver-to-gold-es-cron \
  silver-to-gold-es-manual-$(date +%s) \
  -n default

kubectl get sparkapplications -n default -w | grep silver-to-gold-es

kubectl get pods -n default | grep silver-to-gold-es | grep driver
kubectl logs silver-to-gold-es-abcde-driver -n default
